import { z } from 'zod';
import { guardObjectId } from './guards';
...
export function parseQueryWithSchema<TSchema extends z.ZodTypeAny>(schema: TSchema, payload: unknown) {
	const parsed = schema.safeParse(payload);
	if (!parsed.success) {
		return error(400, parsed.error.issues.map((issue) => issue.message).join('; '));
	}

	return ok(parsed.data, { status: 200, message: 'Valid request query' });
}
